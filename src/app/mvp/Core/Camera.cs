...
using System;                 // ДОБАВЛЕНО: чтобы MathF/Math были гарантированно доступны
...
namespace kuber3d.Core
{
    public class Camera
    {
        ...
        // =========================
        // 1.1) Размеры вьюпорта / Aspect (ДОБАВЛЕНО)
        // =========================
        //
        // Зачем это нужно:
        // - SceneRenderer дергает Camera.Resize(w,h) при Resize окна
        // - Мы храним aspect = w/h внутри камеры
        // - Тогда рендерер может вызывать GetProjectionMatrix() без параметров,
        //   и камера сама использует актуальный aspect.
        ...
        /// <summary>
        /// Текущее соотношение сторон вьюпорта (width/height).
        /// </summary>
        public float Aspect => _aspect;

        /// <summary>
        /// Сообщаем камере новый размер вьюпорта.
        /// Важно: этот метод вызывается при Resize окна/GL-контрола.
        /// </summary>
        public void Resize(int width, int height)
        {
            ...
            _aspect = (float)_viewportWidth / _viewportHeight;
        }
        ...
        /// <summary>
        /// Матрица проекции (перспектива) без параметров.
        /// Использует текущий Aspect, который обновляется через Resize(w,h).
        /// </summary>
        public Matrix4 GetProjectionMatrix()
        {
            return GetProjectionMatrix(_aspect);
        }
